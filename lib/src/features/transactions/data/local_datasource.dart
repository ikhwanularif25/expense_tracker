...
class LocalDataSource {
  ...
  Stream<List<TransactionWithCategory>> watchAllTransactions({String query = ''}) {
    ...
    if (query.isNotEmpty) {
      initialQuery.where(_db.transactions.note.contains(query) |
  ...
  Future<List<drift_db.Category>> getAllCategories() {
    return (_db.select(_db.categories)
          ..orderBy([
            (t) =>
                OrderingTerm(expression: t.isExpense, mode: OrderingMode.desc),
  ...
  Future<void> deleteCategory(int id) async {
    ...
    await (_db.delete(_db.transactions)..where((t) => t.categoryId.equals(id)))
        .go();
  ...
  Stream<List<Map<String, dynamic>>> watchCategorySumByFilter(
    ...
    return query.watch().map((rows) {
      ...
      return totals.entries
          .map((e) =>
              {'category': e.key, 'total': e.value, 'color': colors[e.key]})
          .toList();
    });
  }
    ...
    return combinedTrigger.asyncMap((_) async {
      final budgetRows = await (_db.select(_db.budgets).join([
        innerJoin(_db.categories,
            ...
            ..where(_db.budgets.period.equals(currentPeriod)))
          .get();
      ...
      for (final row in budgetRows) {
        ...
        results
            .add(BudgetWithProgress(budget: budget, category: category, spent: spent));
      }
      return results;
    });
  }
  ...
  Future<void> setBudget(int categoryId, double amount) async {
    ...
    await _db.transaction(() async {
      final existingBudget = await (_db.select(_db.budgets)
            ..where((t) =>
                t.categoryId.equals(categoryId) & t.period.equals(currentPeriod)))
          .getSingleOrNull();
